...
def corrigir_erros_digitacao(df, coluna, lista_valdia):
    for i, valor in enumerate(df[coluna]):
        valor_str = str(valor) if pd.notnull(valor) else valor
        
        if valor_str not in lista_valdia and pd.notnull(valor_str):
            correcao = process.extractOne(valor_str, lista_valdia)[0]
